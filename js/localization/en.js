...
window.TRANSLATIONS.en = {
    ui: {
        ...
        bossdefender_name: 'DEFENDER',
        bosslegion_name: 'LEGI0N',
        boss_4_name: 'EXECUTE',
    ...
    nodes: {
        ...
        restore_point: { name: 'RESTORE POINT', desc: 'When [b]BACKUP SERVER[/b] triggers, restore [color=#37E0F3]40%[/color] health', popup: 'RESTORE POINT LOADED' },
        ...
        emergency_depot: { name: 'EMERGENCY DEPOT', desc: 'Instantly grants [color=cyan]+5000 DATA[/color], [color=#FD7B8B]+3 SHARDS[/color], and [color=white]+3 INSIGHT[/color]', popup: 'RESOURCES ACQUIRED' },
        automated_defense: { name: 'AUTO DEFENSE', desc: 'Core automatically attacks enemies', popup: 'TOWER ATTACK UNLOCKED' },
        focus: { name: 'FOCUS', desc: '[color=#37E0F3]+2[/color] cursor damage', popup: '+2 CURSOR DAMAGE' },
        magnet: { name: 'MAGNETISM', desc: '[color=#37E0F3]+40%[/color] resource pickup range', popup: '+40% PICKUP RANGE' },
        pulse_expansion: { name: 'SIGNAL BOOST', desc: '[color=#37E0F3]+25%[/color] cursor attack size', popup: '+25% SIGNAL STRENGTH' },
        impact: { name: 'IMPACT', desc: '[color=#37E0F3]+2[/color] cursor damage', popup: '+2 CURSOR DAMAGE' },
        integrity: { name: 'INTEGRITY', desc: '[color=#87FF02]+5[/color] MAX HP', popup: '+5 HEALTH' },
        iterative_growth: { name: 'TRIAL & ERROR', desc: 'Permanently gain [color=#87FF02]+2[/color] MAX HP upon death', popup: 'PERMANENT HP GAIN' },
        intensity: { name: 'FIREPOWER', desc: '[color=#37E0F3]+2[/color] core damage', popup: '+2 DAMAGE' },
        coverage: { name: 'COVERAGE', desc: '[color=#37E0F3]+20%[/color] core range', popup: '+20% ATTACK RANGE' },
        diagnostic_analytics: { name: 'DIAGNOSTICS', desc: 'Shows detailed weapon damage stats', popup: 'DIAGNOSTICS UNLOCKED' },
        base_hp_boost: { name: 'AUX POWER', desc: '[color=#37E0F3]+4[/color] core damage', popup: '+4 CORE DAMAGE' },
        system_redundancy_new: { name: 'SYS REDUNDANCY', desc: '[color=#87FF02]+20[/color] MAX HP', popup: '+20 MAX HEALTH' },
        regen: { name: 'AUTO-RESTORE', desc: '[color=#87FF02]+0.4[/color] health regen per second', popup: '+0.4 REGEN' },

        armor: { name: 'ARMOR', desc: 'Reduce damage taken from enemy attacks by [color=#37E0F3]1 per level[/color]', popup: '+1 ARMOR' },
        lightning_weapon: { name: 'LIGHTNING', desc: 'Your core shoots lightning dealing [color=#37E0F3]12[/color] damage every [color=#37E0F3]3s[/color]', popup: 'LIGHTNING WEAPON' },
        shockwave_weapon: { name: 'SHOCKWAVE', desc: 'Your core releases a shockwave dealing [color=#37E0F3]12[/color] AOE damage every [color=#37E0F3]3s[/color]', popup: 'SHOCKWAVE WEAPON' },
        lightning_chain: { name: 'FORK', desc: '[color=#37E0F3]+2[/color] lightning chain targets', popup: '+2 CHAINS' },
        lightning_static_charge: { name: 'INITIAL SHOCK', desc: 'Lightning deals [color=#37E0F3]+4[/color] damage to enemies above [color=#37E0F3]80%[/color] HP', popup: 'INITIAL SHOCK' },
        shockwave_amplifier: { name: 'AMPLIFIER', desc: '[color=#37E0F3]+25%[/color] shockwave range', popup: '+25% RANGE' },
        shockwave_seismic_crush: { name: 'DESTABILIZE', desc: 'Shockwave deals [color=#37E0F3]+1[/color] damage for every [color=#37E0F3]12[/color] missing enemy health', popup: 'SEISMIC CRUSH' },

        clock_speed: { name: 'CLOCK SPEED', desc: '[color=#37E0F3]-5%[/color] core attack cooldown per level', popup: '-5% COOLDOWN' },
        emergency_overclock: { name: 'EMERGENCY OVERCLOCK', desc: 'Core attack speed is [color=#37E0F3]DOUBLED[/color] when below [color=#ff5555]50%[/color] health.', popup: 'EMERGENCY OVERCLOCK' },
        failsafe: { name: 'FAILSAFE', desc: 'When health falls below [color=#ff5555]50%[/color], release an EMP that pushes back enemies', popup: 'FAILSAFE INSTALLED' },
        prismatic_array: { name: 'RECURSIVE ARRAY', desc: '[color=#37E0F3]40%[/color] chance to fire an extra core attack', popup: 'PRISMATIC ARRAY' },
        reveal_map: { name: 'REVEAL MAP', desc: 'Peek a little further into the unknown', popup: ' ' },
        test_defenses: { name: 'SANDBOX', desc: 'Unlock the "COMBAT TEST" button for practice', popup: 'SANDBOX UNLOCKED' },
        ...
        memory_leak: { name: 'MEMORY LEAK', desc: 'Cursor attacks cause bosses to drop [color=cyan]DATA[/color] from [color=#37E0F3]10% of damage[/color]', popup: 'MEMORY LEAK DETECTED' },
        bomb: { name: 'BOMB', desc: 'Acquire a [color=#ffb300]BOMB[/color]. Bombs deal +40 cursor damage in a huge radius.', popup: '+1 BOMB' },
        bomb_2: { name: 'EXTRA BOMB', desc: 'Acquire another [color=#ffb300]BOMB[/color]. Total max uses increased.', popup: '+1 BOMB' },
        lethal_explosive: { name: 'LETHAL EXPLOSIVE', desc: 'Bombs deal [color=#37E0F3]+30[/color] damage', popup: '+30 BOMB DAMAGE' },
        data_compression: { name: 'ZIP STREAM', desc: 'Enemies drop [color=#37E0F3]+20%[/color] more [color=cyan]DATA[/color]', popup: '+20% DATA DROPS' },
        ...
        manual_protocol: { name: 'MANUAL CONTROL', desc: 'Manual cursor attack. Click to fire. Stores [color=#37E0F3]2[/color] charges.', popup: 'MANUAL PROTOCOL' },
        broadcast_protocol: { name: 'BROADCAST', desc: '[color=#37E0F3]+25%[/color] cursor attack size.', popup: 'BROADCAST PROTOCOL' },
        manual_pulse_child_1: { name: 'CHARGE BUFFER', desc: '[color=#37E0F3]+1[/color] max pulse charges', popup: 'CAPACITY INCREASED' },
        manual_pulse_child_1_1: { name: 'ISOLATION', desc: '[color=#37E0F3]+50%[/color] cursor damage per level vs single targets', popup: 'ISOLATION PROTOCOL' },
        manual_pulse_child_1_2: { name: 'FAST RELOAD', desc: 'Charges refill [color=#37E0F3]25%[/color] faster', popup: 'RELOAD OPTIMIZED' },
        area_saturation: { name: 'SATURATION', desc: '[color=#37E0F3]+1[/color] cursor damage per additional enemy hit.', popup: 'AREA SATURATION' },
        aftershock: { name: 'AFTERSHOCK', desc: 'Fires a secondary, larger AOE attack dealing [color=#37E0F3]4/8[/color] damage', popup: 'AFTERSHOCK' },
        colossal_cursor: { name: 'COLOSSAL CURSOR', desc: '[color=#37E0F3]+50%[/color] cursor attack size', popup: 'COLOSSAL CURSOR' },
        rocket: { name: 'ROCKET', desc: 'Core now fires a powerful [color=#37E0F3]AOE[/color] explosive', popup: 'ROCKET UNLOCKED' },
        assault: { name: 'ASSAULT', desc: 'Core now fires a burst of [color=#37E0F3]3[/color] weaker projectiles [color=#ffb300](-2 damage)[/color]', popup: 'ASSAULT UNLOCKED' },
        assault_clip_size: { name: 'CLIP SIZE', desc: '[color=#37E0F3]+1[/color] projectile fired per burst', popup: 'CAPACITY INCREASED' },
        suppressive_fire: { name: 'SUPPRESSION', desc: '[color=#37E0F3]30%[/color] chance to heavily [color=#37E0F3]slow[/color] non-boss enemies', popup: 'SUPPRESSIVE FIRE ENABLED' },
        rocket_he_compound: { name: 'HE COMPOUND', desc: '[color=#37E0F3]+30%[/color] rocket explosion radius', popup: 'EXPLOSION RADIUS INCREASED' },
        rocket_chain_reaction: { name: 'CHAINED BLAST', desc: 'Enemies killed by explosions trigger a [color=#37E0F3]secondary blast[/color] after 0.1s', popup: 'CHAIN REACTION ENABLED' },
        resonance: { name: 'RESONANCE', desc: 'Every 4th cursor attack gains RESONANCE, dealing [color=#37E0F3]double damage[/color]', popup: 'RESONANCE ACTIVE' },
        amplitude: { name: 'AMPLITUDE', desc: 'Resonance damage increased to [color=#37E0F3]triple damage[/color] (x3).', popup: 'AMPLITUDE INCREASED' },
        ...
        artillery: { name: 'ARTILLERY', desc: 'Fires large AOE artillery blast every [color=#37E0F3]6s[/color] at random enemy', popup: 'ARTILLERY ACTIVE' },
        lore_1: {
            name: 'ARCHIVE',
        ...
        junk_data_1: { name: 'COLLECT DATA', desc: 'Instantly grants [color=#37E0F3]+5[/color] [color=cyan]DATA[/color]', popup: '+5 DATA' },
        ...
        impulse: { name: 'IMPULSE', desc: 'Cursor deals [color=#37E0F3]+50%[/color] damage against [color=#37E0F3]FULL HEALTH[/color] enemies', popup: 'IMPULSE' },
        ...
        artillery_volley: { name: 'TRIPLE VOLLEY', desc: 'Fire [color=#37E0F3]+2[/color] additional artillery strikes', popup: 'TRIPLE VOLLEY' },
        artillery_first_strike: { name: 'FIRST STRIKE', desc: 'Artillery deals [color=#37E0F3]+20[/color] damage per lvl to enemies above [color=#37E0F3]80%[/color] HP', popup: 'FIRST STRIKE' },
        ...
        combat_shield: { name: 'COMBAT SHIELD', desc: 'Create a controllable shield with [color=#37E0F3]10[/color] health', popup: 'SHIELD DEPLOYED' },
        combat_shield_hp: { name: 'REINFORCED PLATING', desc: 'Grants [color=#37E0F3]+10[/color] health to the combat shield', popup: '+10 SHIELD HEALTH' },
        sword: { name: 'SWORD', desc: 'Stabs in a long straight line every [color=#37E0F3]2s[/color]', popup: 'SWORD SYSTEM ONLINE' },
        scythe: { name: 'SCYTHE', desc: 'Swings in a broad sweeping motion every [color=#37E0F3]2s[/color]', popup: 'SCYTHE SYSTEM ONLINE' },
        scythe_harvest: { name: 'HARVEST', desc: 'Each enemy killed by the Scythe restores [color=#87FF02]1 HP[/color]', popup: 'HARVEST ACTIVE' },
        scythe_lethality: { name: 'LETHALITY', desc: 'Scythe deals [color=#37E0F3]+10[/color] damage per level to enemies below [color=#37E0F3]50%[/color] HP', popup: 'LETHALITY ACTIVE' },
        sword_lunge: { name: 'LUNGE', desc: 'Increases the length and range of the sword attack by [color=#37E0F3]+25%[/color]', popup: 'LUNGE ACQUIRED' },
        sword_flurry: { name: 'FLURRY', desc: 'Performs two extra strikes at [color=#37E0F3]60%[/color] power each', popup: 'FLURRY ACQUIRED' },
        ...
        physical_anchor: { name: 'PURCHASE SERVERS', desc: '[color=#37E0F3]+40[/color] max health', popup: '+40 MAX HEALTH' },
        ...
        instability_mark: { name: 'INSTABILITY', desc: 'Cursor attacks [color=#df66ff]mark[/color] enemies. Next non-cursor hit deals [color=#df66ff]+10[/color] damage.', popup: 'INSTABILITY MARK' },
        milestones: { name: 'MILESTONES', desc: 'Unlocks the Milestones menu' },
        crescendo: { name: 'CRESCENDO', desc: 'Resonance attacks gain [color=#37E0F3]+100%[/color] size', popup: 'AMPLIFIED' },
        ...
        proxy_routing: { name: 'PROXY ROUTING', desc: 'Reduces all enemy damage by [color=#37E0F3]20%[/color]', popup: 'PROXY ROUTING ENABLED' },
        peak_performance: { name: 'PEAK PERFORMANCE', desc: '[color=#37E0F3]+4[/color] core damage while above [color=#37E0F3]90%[/color] health', popup: '+4 CORE DAMAGE' },
        data_mining: { name: 'DATA MINING', desc: 'Core attacks extract [color=#37E0F3]20%[/color] of damage as [color=cyan]DATA[/color] from Bosses', popup: 'DATA MINING INITIALIZED' },
        parallel_processing: { name: 'PARALLEL PROCESSING', desc: '[color=#37E0F3]+2[/color] Core damage\n[color=#37E0F3]+2[/color] Cursor damage', popup: 'PROCESSING CAPABILITIES SYNCED' },
        farsight: { name: 'FARSIGHT', desc: '[color=#37E0F3]+20%[/color] core range', popup: '+20% ATTACK RANGE' },
        label: {
            lore: 'LORE',
            gate: 'GATE',
            plus_data: '+DATA',
    ...
    hud: {
        ...
        shard_desc: 'A fragmented piece of system architecture. Unlocks powerful abilities.',
