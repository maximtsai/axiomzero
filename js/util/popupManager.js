/**
 * @fileoverview Generic modal popup system.
 * Provides showPopup(), showNineSlicePopup(), and showYesNoPopup().
 * Only requires 'white_pixel' (always in Phase 1 preload).
 * @module popupManager
 */
...
//
// Internal helper that creates overlay, blocker, background, title, body, and
// action buttons. Both showPopup() and showNineSlicePopup() delegate to this.

/**
 * Build a complete popup with overlay, blocker, background, title, body, and buttons.
 * @param {Object}   opts
 * @param {Function} opts.createBG - Receives (W, H, depth, fast) and returns the popup background object.
 * @param {number}   opts.boxWidth - Width used for body text word wrap.
 * @param {string}   [opts.title]
 * @param {string}   [opts.body]
 * @param {Array}    [opts.buttons] - Array of { text, onClick, primary }.
...
function _buildPopup({ createBG, boxWidth, title = '', body = '', buttons = [], depth = _POPUP.DEPTH, fast = false }) {
    if (typeof audio !== 'undefined') audio.play('retro1', 1.0);
    ...
    // Title
    const titleObj = PhaserScene.add.text(W, H - 10 - popupBG._popupHalfH + 32, title, {
    ...
    const bodyObj = PhaserScene.add.text(W, H - 10, body, {
        ...
        align: 'center', wordWrap: { width: boxWidth - 48 },
    }).setOrigin(0.5, 0.5).setDepth(depth + 2).setAlpha(0);
...
/**
 * Show a generic popup with a white_pixel background box.
 * @param {Object}   opts
 * @param {string}   [opts.title]   - Popup title text.
 * @param {string}   [opts.body]    - Popup body text.
 * @param {Array}    [opts.buttons] - Array of { label, onClick, primary }.
 * @param {number}   [opts.depth]   - Phaser depth layer.
 * @param {boolean}  [opts.fast]    - Skip fade-in animation.
 * @returns {Function} closePopup - Call to dismiss the popup.
 */
...
/**
 * Show a popup with a nineslice-scaled background texture.
 * @param {Object} opts - Same as showPopup, plus texture/atlas/cornerSize/width/height.
 * @returns {Function} closePopup
 */
...
/**
 * Convenience wrapper: two-button Yes/No popup.
 * @param {string}   yesText   - Label for Yes button.
 * @param {string}   noText    - Label for No button.
 * @param {string}   [titleText] - Popup title.
 * @param {string}   [bodyText]  - Popup body.
 * @param {Function} [onYes]     - Callback when Yes is clicked.
 * @param {boolean}  [superFast] - Skip fade-in.
 */
