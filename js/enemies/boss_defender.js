// js/enemies/boss_defender.js — Phase 2 boss (MVC).
...
class BossDefenderModel extends BossModel {
    constructor(levelScalingModifier = 1) {
        ...
        this.size = 95;
        this.bossId = 'bossdefender';

        this.state = BOSS_DEFENDER_STATES.TRAVEL;
        ...
        this.projectileDamage = BOSS_DEFENDER_CONFIG.PROJECTILE_DAMAGE;
    ...
    activate(x, y, config = {}) {
        ...
        this.circlingTime = 0;
        this.attackCooldown = 0;
        this.projectileDamage = config.projectileDamage || BOSS_DEFENDER_CONFIG.PROJECTILE_DAMAGE;
        this.attackCount = 0;
        this.setupDelay = 0;
    ...
    update(dt) {
        ...
        if (this.state === BOSS_DEFENDER_STATES.TRAVEL) {
            ...
            if (dist < BOSS_DEFENDER_CONFIG.ENTRY_RADIUS) {
                this.state = BOSS_DEFENDER_STATES.CIRCLING;
                this.circlingTime = 0;
                this.attackCount = 0; // Reset circling attack count
                this.speed = GAME_CONSTANTS.ENEMY_BASE_SPEED * BOSS_DEFENDER_CONFIG.CIRCLING_SPEED_MULT;
                if (typeof audio !== 'undefined') {
                    this.activeSound = audio.play('ship_creak', 1.05);
                }
            } else {
                const targetSpeed = GAME_CONSTANTS.ENEMY_BASE_SPEED * BOSS_DEFENDER_CONFIG.MAX_SPEED_MULT;
            ...
            this.syncVelocity();
        } else if (this.state === BOSS_DEFENDER_STATES.CIRCLING) {
            this.circlingTime += dt;
            // 1. Calculate desired velocity vector
            const targetOrbitRadius = BOSS_DEFENDER_CONFIG.ORBIT_RADIUS;
            ...
            this.syncVelocity();
        } else if (this.state === BOSS_DEFENDER_STATES.SETUP) {
            if (this.setupDelay > 0) {
                ...
                if (this.setupDelay <= 0 && this.setupTarget === null) {
                    this.setupDelay = 0;
                    this.speed = GAME_CONSTANTS.ENEMY_BASE_SPEED * BOSS_DEFENDER_CONFIG.MAX_SPEED_MULT; // Return to max speed
                    const futureX = this.x + this.vx * 3;
                    const tPos = tower.getPosition();
                    if (futureX < tPos.x) {
                        this.setupTarget = { x: BOSS_DEFENDER_CONFIG.FLANK_X_OFFSET, y: tPos.y };
                    } else {
                        this.setupTarget = { x: GAME_CONSTANTS.WIDTH - BOSS_DEFENDER_CONFIG.FLANK_X_OFFSET, y: tPos.y };
                    }
                }
                ...
                if (targetDist < 100) {
                    this.state = BOSS_DEFENDER_STATES.AIMING;
                    if (typeof audio !== 'undefined') {
                    ...
                    const avoidRadius = BOSS_DEFENDER_CONFIG.TOWER_AVOIDANCE_RADIUS;
                    if (dist < avoidRadius) {
        ...
        } else if (this.state === BOSS_DEFENDER_STATES.AIMING) {
            ...
            if (Math.abs(tDiff) < 0.01) {
                ...
                this.state = BOSS_DEFENDER_STATES.BOMBARD;
                this.pastTurretRotationChange = 0;
        ...
        } else if (this.state === BOSS_DEFENDER_STATES.BOMBARD) {
        ...
        if (this.state !== BOSS_DEFENDER_STATES.AIMING && this.state !== BOSS_DEFENDER_STATES.BOMBARD) {
            this.turretRotation = this.baseRotation;
        }
...
class BossDefenderView extends EnemyView {
    constructor() {
...
class BossDefender extends Boss {
    constructor(levelScalingModifier = 1) {
        super(levelScalingModifier);
        this.model = new BossDefenderModel(levelScalingModifier);
        this.view = new BossDefenderView();
    ...
    activate(x, y, scaleFactor = 1.0) {
        ...
        const bossHealth = 295;

        super.activate(x, y, {
            maxHealth: bossHealth,
            damage: GAME_CONSTANTS.ENEMY_BASE_DAMAGE, // Static hull damage
            selfDamage: 0,
            speed: GAME_CONSTANTS.ENEMY_BASE_SPEED * BOSS_DEFENDER_CONFIG.MAX_SPEED_MULT,
            initialSpeedMult: this.model.initialSpeedMult,
            rampDuration: this.model.rampDuration,
            size: this.model.size,
            projectileDamage: BOSS_DEFENDER_CONFIG.PROJECTILE_DAMAGE // Static projectile damage
        });
        ...
        PhaserScene.time.delayedCall(1000, () => {
            messageBus.publish('BossAnnounceText', { msg1: t('ui', 'boss_prefix'), msg2: t('ui', 'bossdefender_name') });
        });
    }
    ...
    update(dt) {
        ...
        if (this.model.state === BOSS_DEFENDER_STATES.BOMBARD) {
        ...
        if (this.model.state === BOSS_DEFENDER_STATES.CIRCLING && this.model.circlingTime >= 4) {
            if (this.model.attackCooldown <= 0) {
                ...
                if (this.model.attackCount >= 3) {
                    ...
                    this.model.state = BOSS_DEFENDER_STATES.SETUP;
                }
            }
        }
    }
    ...
    _fireProjectilePair() {
        this.model.attackCooldown = BOSS_DEFENDER_CONFIG.ATTACK_COOLDOWN;
