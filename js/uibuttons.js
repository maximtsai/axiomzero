...
function _showOptionsPopup() {
    ...
    const fullBg = PhaserScene.add.nineslice(W - width / 2 + 230, visualHeaderY + 90, 'ui', 'glow_btn_9slice.png', 140, 56, 20, 20, 20, 20);
    fullBg.setDepth(depth + 3).setScrollFactor(0);
    elements.push(fullBg);

    const fullBtn = new Button({
        normal: { ref: 'white_pixel', x: W - width / 2 + 230, y: visualHeaderY + 90, alpha: 0.001, scaleX: 65, scaleY: 28 },
    ...
    const minBg = PhaserScene.add.nineslice(W - width / 2 + 380, visualHeaderY + 90, 'ui', 'glow_btn_9slice.png', 140, 56, 20, 20, 20, 20);
    minBg.setDepth(depth + 3).setScrollFactor(0);
    elements.push(minBg);

    const minBtn = new Button({
        normal: { ref: 'white_pixel', x: W - width / 2 + 380, y: visualHeaderY + 90, alpha: 0.001, scaleX: 65, scaleY: 28 },
        disable: { ref: 'white_pixel', alpha: 0.001 },
        onHover: () => {
